...
| 2 | Eliminación de un círculo | Solo el **creador** puede eliminar el círculo. Los miembros solo pueden abandonarlo. Al eliminarse, todos los miembros quedan desvinculados y regresan a "Aún no estás en un círculo". | 🔗 | ⚠️ | Test pasando 2026-03-20 pero con validación incompleta: el campo `creatorId` no existe aún en Firestore. Re-ejecutar tras implementar Brecha 2 de `WORK_PLAN_CIRCLE_RULES.md`. |
...
| 7 | La UI no ofrece "Salir del Círculo" sin eliminar cuenta | El usuario dentro de un círculo no encuentra ninguna opción para salir sin eliminar su cuenta (Settings u otra pantalla) | 👁 | | Pendiente. Verificar después de implementar Brecha 1 de `WORK_PLAN_CIRCLE_RULES.md`. |
...
| 10 | Solicitud de ingreso — solicitante queda en estado pendiente | B ingresa código y ve pantalla de espera. En Firestore: `joinRequests/{uid}` con status "pending" y `users/{uid}.pendingCircleId` seteado. | 👁 | | Pendiente. Requiere implementar `WORK_PLAN_JOIN_APPROVAL.md`. |
...
| 6 | T2.7 | 2 | La UI no ofrece "Salir del Círculo" sin eliminar cuenta | Requiere Brecha 1 implementada. Verificar que la opción no existe en Settings ni en ninguna otra pantalla. |
| 7 | T2.8 | 2 | Eliminar cuenta siendo creador — otros miembros activos | Requiere Brecha 2. Ver pasos en la fila T2.8 de Fase 2. |
| 8 | T2.9 | 2 | Eliminar cuenta siendo miembro común — creador sigue activo | Requiere Brecha 2. Ver pasos en la fila T2.9 de Fase 2. |
| 9 | T2.10 | 2 | Solicitud de ingreso — solicitante queda en estado pendiente | Requiere `WORK_PLAN_JOIN_APPROVAL.md` implementado. |
| 10 | T2.11 | 2 | Creador aprueba solicitud | Requiere `WORK_PLAN_JOIN_APPROVAL.md` implementado. |
| 11 | T2.12 | 2 | Creador rechaza solicitud | Requiere `WORK_PLAN_JOIN_APPROVAL.md` implementado. |
| 12 | T2.13 | 2 | Solicitante rechazado intenta unirse al mismo círculo | Requiere `WORK_PLAN_JOIN_APPROVAL.md` implementado. Opción A. |
| 13 | T2.14 | 2 | Solicitante rechazado puede crear su propio círculo | Requiere `WORK_PLAN_JOIN_APPROVAL.md` implementado. |
| 14 | T2.15 | 2 | Segunda solicitud bloqueada mientras hay una pendiente activa | Requiere `WORK_PLAN_JOIN_APPROVAL.md` implementado. |
| 15 | T3.20.1 | 3 | Con zonas activas — usuario entra a una zona | Requiere GPS activo y zona configurada en el círculo. |
| 16 | T3.20.2 | 3 | Con zonas activas — usuario sale de una zona | Verificar cambio automático a "En camino". |
| 17 | T4.1 | 4 | App minimizada — ícono visible en barra superior | Inspección visual en dispositivo físico. |
| 18 | T4.2 | 4 | App minimizada sin cerrar sesión — modo silent activo | Verificar que el ícono persiste y el servicio sigue vivo. |
| 19 | T4.3 | 4 | Cerrar sesión — ícono desaparece de barra superior | Comportamiento a confirmar. |
| 20 | T5.2 | 5 | Quick Actions reflejadas en shortcuts nativos del SO | Inspección visual: mantener pulsado el ícono de la app en el launcher. |
