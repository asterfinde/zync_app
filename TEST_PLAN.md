...
| 11b | T3.30.3 | 3 | Intento de seleccionar zona predefinida manualmente cuando el círculo tiene zonas configuradas | Precondición: círculo con al menos una zona configurada. Pasos: tocar propio avatar → selector abre → botones 🏠🏫🏢🏥 con opacidad reducida → tocar cualquiera → modal "Acción no permitida" aparece → tocar "Entendido" → estado no cambió. |
