...
| 3 | Registro fallido — email ya registrado | Mensaje "Este correo ya tiene una cuenta registrada. Inicia sesión." | 👁 | ✅ | Lógica correcta. Test automatizado falla por teclado virtual del test anterior que bloquea el tap en modo integración. Retomar si se resuelve el reset de teclado entre tests. |
...
| 6 | Login fallido — contraseña incorrecta | Mensaje "Correo o contraseña incorrectos. Verifica que te has registrado e intenta de nuevo." | 👁 | ✅ | Lógica correcta. Test automatizado falla porque `pumpAndSettle(10s)` descarta el SnackBar (duración 4s) antes del `expect`. Retomar ajustando timing. |
