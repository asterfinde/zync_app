...
| 2026-07-02 | Búsqueda de lugares: `locationRestriction` definitivo, no `locationBias` | Se probó `locationBias` para intentar traer homónimos no listados (ej. "Calle los Molles" en Ate) — sin efecto: la calle no está indexada en Places Autocomplete (New), ningún parámetro de cliente la trae. `locationRestriction` se revirtió porque además diverge del filtro duro de §10.1. La respuesta real a homónimos/lugares no indexados es el **marcado manual** (tap/drag en el mapa), con caption de descubribilidad agregado en `zone_form.dart`. Cierra `[DT-PLACES-NEAREST]`. |
| 2026-07-02 | Cliente Places SDK se invalida tras agotar reintentos de red | Bug crónico: tras Doze/idle prolongado, el `PlacesClient` nativo cacheado indefinidamente en `PlacesSdkSearchService` quedaba con una conexión TCP muerta en el pool — cada búsqueda fallaba hasta el keep-alive (~5 min). Fix: `_withRetry` reintenta con backoff (400ms, 1200ms) y descarta `_client` (`= null`) si todos los intentos fallan por red, forzando un pool fresco en el siguiente intento. Validado en device (recuperación en segundos, no minutos). |
| 2026-07-02 | Sem 8 Geofencing BC: `GeofencingService` (adapter) vs `DetectZoneTransition` (use case) | `GeofencingService` queda como adapter delgado sobre `Geolocator` (solo ciclo GPS start/stop/check). Toda la lógica de dominio (matching de zona solapada, debounce, transición entrada/salida, suppress-on-reopen, guard Bug E) se extrajo a `DetectZoneTransition` (`contexts/geofencing/application/use_cases`), un use case testeable sin depender de GPS. Patrón productor (`DetectZoneTransition`) / consumidor (`ApplyGeofenceStatus`, ya existente) sobre el `DomainEventBus`. Cierra el entregable "thin adapter" de §3.8 del plan de refactor. |
